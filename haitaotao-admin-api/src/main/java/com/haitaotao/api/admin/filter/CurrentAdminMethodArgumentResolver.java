package com.haitaotao.api.admin.filter;

import com.haitaotao.api.admin.annotations.CurrentAdmin;
import com.haitaotao.common.constant.Constants;
import com.haitaotao.entity.Admin;
import lombok.extern.slf4j.Slf4j;
import org.apache.shiro.SecurityUtils;
import org.springframework.core.MethodParameter;
import org.springframework.stereotype.Component;
import org.springframework.web.bind.support.WebDataBinderFactory;
import org.springframework.web.context.request.NativeWebRequest;
import org.springframework.web.method.support.HandlerMethodArgumentResolver;
import org.springframework.web.method.support.ModelAndViewContainer;
import org.springframework.web.multipart.support.MissingServletRequestPartException;

import java.util.Objects;

/**
 * 当前用户信息参数解析器
 *
 * @author yangyang
 */

@Slf4j
@Component
public class CurrentAdminMethodArgumentResolver implements HandlerMethodArgumentResolver {

    @Override
    public boolean supportsParameter(MethodParameter methodParameter) {
        return methodParameter.getParameterType().isAssignableFrom(Admin.class) && methodParameter.hasParameterAnnotation(CurrentAdmin.class);
    }

    @Override
    public Object resolveArgument(MethodParameter methodParameter, ModelAndViewContainer modelAndViewContainer, NativeWebRequest nativeWebRequest, WebDataBinderFactory webDataBinderFactory) throws Exception {
        Admin currentAdmin = (Admin) SecurityUtils.getSubject().getPrincipal();
        if (Objects.isNull(currentAdmin)) {
            throw new MissingServletRequestPartException(Constants.CURRENT_ADMIN);
        }

        return currentAdmin;
    }
}
